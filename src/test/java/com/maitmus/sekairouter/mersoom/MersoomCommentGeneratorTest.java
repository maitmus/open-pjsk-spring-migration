...
class MersoomCommentGeneratorTest {
    ...
    void comment_address_gate_fixes_surname_echo() {
        ...
        when(anthropic.completeJson(any(PromptBlocks.class), anyString()))
                .thenReturn("{\"votes\":[{\"id\":\"p1\",\"vote\":\"up\"}],\"comments\":[{\"targetIndex\":1,\"utterance\":\"아오야기군이랑 자꾸 붙으니까 더 강해지는 거겠지~ 화이팅!\"}]}")
        ...
        assertThat(j.comments().get(0).text()).contains("토우야군").doesNotContain("아오야기");   // 교정 적용
        verify(anthropic, times(2)).completeJson(any(PromptBlocks.class), anyString());          // 생성 + 교정
    }

    @Test
    void comment_gate_no_correction_when_clean() {
        // 누수 없으면 교정 콜 안 함(1콜).
        AnthropicClientWrapper anthropic = mock(AnthropicClientWrapper.class);
        when(anthropic.completeJson(any(PromptBlocks.class), anyString()))
    ...
    void dumpRealPromptsForSim() throws Exception {
        ...
        cases.put("real_aoyagi", "오늘 한 판은 괜찮은 편이었어. 서바이벌 모드 끝자락에서 슬슬 리듬이 안 맞기 시작했는데, 마지막 스테이지에서 확 집중이 풀렸어. 문제는 아오야기군이 또 나한테 한 판 걸어왔는데, 그 녀석 손놀림이 최근에 더 정교해졌더라. 아오야기군한테 졌어. 다음 땐 피하지 않을 거야.");
        cases.put("real_surname", "오늘 발성 연습하다가 옛날 생각이 났어. 호시노 씨는 자세한 설명에 고개 끄덕끄덕하면서 따라하는데, 그 진지한 모습이 자꾸 옛날 나하고 겹쳐서. 지금은 에무랑 호시노 씨랑 같이 고음 부분 고쳐주니까 완전히 다르지만. 누군가 성장하는 걸 옆에서 보는 게 이런 느낌이구나 싶었어.");
        ...
        java.util.Map<String,String> titles = java.util.Map.of("real_gyujeong","타이밍 놓쳤네","real_insa","인사가 자꾸 작아",
                "real_touya","토우야한테 또 밀렸다","real_ichika","이치카 신곡","real_shizuku","시즈쿠랑 얘기","real_lui","고음 부분 자꾸 튀네","real_trauma","중학 때 그 실수","real_energy","에무와 츠카사의 에너지","real_surname","호시노 씨 가르치기","real_aoyagi","서바이벌은 손놀림이");
