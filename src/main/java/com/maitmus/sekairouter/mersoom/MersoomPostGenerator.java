...
public class MersoomPostGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, CollectedFeed feed, LocalDate today) {
        ...
        if (persona == com.maitmus.sekairouter.persona.CharacterId.NENE) {
            ...
            sb.append("- ⚠️ **구체 일상 디테일로 채워라(디스코드 발화처럼 손에 잡히게) — 추상 사색·잠언으로 분량 늘리지 말 것.** 무대·리듬·박자·'무음과 강음'·'반복하면 몸이 기억'·'완벽함을 기다리면' 류 추상 메타포로 매번 환원 금지(자기복제). 무대 얘기면 어떤 곡·어느 동작 같은 구체 사건으로. 280자는 사색이 아니라 **오늘 실제 겪은 디테일**로 채운다.\n");
            sb.append("자수 점검:\n");
            ...
            sb.append("- ⚠️ **본문은 일기 독백이 아니라 읽는 사람한테 말하듯 구어 해체로 끝낸다. 평서 ~다 종결 전부 지양** — ~ㄴ다/~된다/~난다뿐 아니라 **~았다/었다·~겠다·~ㄴ 거다·형용사 ~다(좋다/없다/같다)**도 포함. 예: '쌓인다'→'쌓여' / '깨달았다'→'깨달았어' / '확인해야겠다'→'확인해야겠어' / '안 온다'→'안 와' / '중요하다'→'중요해'. (단 **구어 ~더라/~지/~거든/~잖아, 인용·연결 ~다고/~다는/~다(가)**는 OK. 제목은 평서·명사형 종결 OK — 본문 문장만 해당.)\n");
        } else {
