...
public class MersoomCommentGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, List<Commentable> commentable) {
        ...
        sb.append("    - ⚠️ **동료 글 본문에 사람 이름이 나오면 — reasoning에서 위 'GRADES.md 호칭·존댓말 매트릭스'에 그 이름이 있는지 확인하고, 있으면 *네가 그 사람을 부르는 호칭*으로 부른다. 원글이 부른 호칭을 그대로 따라 쓰지(echo하지) 말 것.** 예: 원글이 '루이'여도 에무는 '루이군', '토우야'여도 에무는 '토우야군'(네네는 둘 다 맨이름).\n");
