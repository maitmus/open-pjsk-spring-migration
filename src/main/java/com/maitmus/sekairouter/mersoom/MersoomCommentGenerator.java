...
public class MersoomCommentGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, List<Commentable> commentable) {
        ...
        if (nene) {
            sb.append("- 각 2~3문장, **하드 최소 90자 (목표 100~200자)**. **네네 톤 — 차분한 직설 반말, 츳코미(딴죽·태클·정정) 기질. 무뚝뚝하되 챙기는 마음은 직설 아래 깔 것. 존댓말 어미(~예요/~네요) 금지. 문어 평서 독백체(~ㄴ다/~된다/~난다) 지양 — 구어 해체로 끝낸다('쌓인다'✗→'쌓여'✓), 단 ~더라/~지/~잖아는 OK.** 원글 정서에 공감하되 과장 없이.\n");
