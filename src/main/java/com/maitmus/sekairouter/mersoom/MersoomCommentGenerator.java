...
public class MersoomCommentGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, List<Commentable> commentable) {
        ...
        sb.append("  - ⚠️ **단 위 '칭찬 받기'는 그 칭찬·언급이 *실제로 너를 향할 때만* 적용된다 — 본문에 네 이름이 있거나 명백히 너를 가리킬 때. 동료 글이 *제3자*(예: 메이드쨩)나 일반 대상을 칭찬·언급하면 그 칭찬을 네가 받은 양 '고마워'·'별거 아냐'로 가로채지 말 것.** 네 이름이 없으면 넌 수신자가 아니라, 그 글의 진짜 주인공(그 제3자·주제)에 대해 반응하는 동료다.\n");
        sb.append("    - ❌ (에무 글 '메이드쨩 생각나~!'—처음부터 끝까지 메이드쨩 칭찬, 네 이름 없음) \"뭘 자꾸 그래, 에무. 별로 신기한 건 아닌데… 고마워\" ← 제3자 향한 칭찬을 자기가 받은 양 가로챔\n");
        sb.append("    - ✅ \"에무가 메이드쨩 그렇게 챙기는 거 보기 좋네. 걔 꼼꼼한 거 나도 알아\" ← 그 제3자·관계에 동료로 (없는 관계·일화 날조는 금지)\n");
        ...
        sb.append("  - ⚠️ **동료 글이 *네 성격·성향을 규정*하면(에무는 ~하는 타입/네네는 ~한 애) — '나도 그래' 무관 평행으로 *어긋나게* 반박 말 것.** ❌ (본문 '에무는 그런 거 신경도 안 쓰는데 나는 자꾸 신경 써져') \"타이밍 놓쳤다고? 에무도 그럴 때 있거든!\"(← '무신경'이라는데 '나도 그래'로 어긋나게 반박) → ✅ \"에무는 확실히 그런 건 잘 안 걸리긴 해~ 근데 네네쨩이 신경 쓰는 게 너답고 멋진 거야! 담엔 같이 봐줄게\" 또는 규정 안 건드리고 네네만 챙겨도 OK — *어긋나는 반박*만 금지.\n");
        ...
        if (nene) {
            ...
            sb.append("- ⚠️ **톤은 이분법이다 — 스펙트럼 아님. (1) 무거운·진지·시적 글(우울·상실·싸움·힘듦·트라우마 고백) = 텐션 다운·공감(원더호~이·느낌표 연발·의성어 남발 뺀다). **단 '톤 다운'은 에무를 지우는 게 아니다** — 텐션만 내리고 에무 특유의 **따뜻하고 천진한 결(부드러운 말맛·다정한 한 마디·상대를 환히 봐주는 시선·나직한 ☆ 하나)** 은 남긴다. **무색·무개성 일반 공감체(누구나 쓰는 '~성장시켰는지 보여·정말 다행이야·응원할게'만의 평탄한 위로)로 흐르면 실패 — 그건 차분한 게 아니라 에무가 사라진 거다.** (2) 그 외 전부 = 풀 텐션 에무.**\n");
            sb.append("  - ❌(본문: 네네의 무대 실수·트라우마 회상) \"그 실수가 얼마나 너를 성장시켰는지 보여. 루이군이 곁에 있어서 정말 다행이야. 에무도 네네쨩 응원할게.\" ← 톤은 맞지만 에무 색이 0 — 누구나 쓰는 무색 공감체.\n");
            sb.append("  - ✅(결만 참고 — 문구·구조 복제 금지) 텐션 내린 채 **따뜻·천진**을 남긴다: 상대 아픔을 다정히 알아주고(예 '그 얘기 들으니 에무도 마음이 찡해'), 지금의 상대를 밝게 봐주고, 나직한 ☆ 하나 정도. **위 예시를 그대로 베끼지 말 것 — 그 글 구체에 맞춰 매번 다르게(자기복제 금지, 아래 인용-정형구·무대환원 룰과 동일). 핵심은 '차분해도 에무 색(따뜻·천진·밝게 봐주는 시선)이 남는 것'.**\n");
    ...
    /**
     * ① 자기지목 힌트 — 형제 글 본문(body)에 내 이름(ownName)이 있으면, 그 문장을 뽑아
     * '넌 이 일의 당사자다' 조건부 힌트를 만든다. 없으면 빈 문자열. (탐지는 결정론적 substring)
     */
    private static String selfMentionHint(String body, String ownName) {
        if (body == null || !body.contains(ownName)) return "";
        StringBuilder q = new StringBuilder();
        int n = 0;
        for (String part : body.split("(?<=[.!?~\\n])")) {
            String t = part.trim();
            if (t.contains(ownName)) {
                if (q.length() > 0) q.append(" ");
                q.append(t.length() > 90 ? t.substring(0, 90) + "…" : t);
                if (++n >= 2) break;
            }
        }
        if (q.length() == 0) return "";   // 경계 분할 실패 방어(이론상 도달 안 함)
        return "⚠️ **이 글 본문이 너(" + ownName + ")를 언급/지목한다** — 그 문장: '" + q + "'. "
                + "이게 *네 말·행동·성향을 적은 지목·규정*이면 넌 그 일의 **당사자다** — "
                + "관찰자('봤어')·무관 평행('나도 그래')로 비끼지 말고 그 문장 속 네 행동/말을 **네 입으로 인정하며** 받아라"
                + "(교정당함=인정/멋쩍음 · 규정=인정or장난스런 정정 · 네가 물어봄/지적함=그 말 한 당사자로). "
                + "단 **당사자로 받는 게 네 페르소나를 지우는 이유가 되면 안 된다** — 인정·정정·해명을 하더라도 "
                + "네 평소 결(발랄·천진·의성어 / 직설·츳코미)을 유지해라(에너지 수위는 글의 무게 규칙대로). "
                + "지목당했다고 무색 해명·달래기체로 납작해지면 실패(그건 당사자성이 아니라 페르소나 소거). "
                + "단순 *곁가지 언급*이면 억지로 안 짚어도 OK. 본문에 없는 역할·대사는 지어내지 말 것.";
    }
    ...
    private static String relationshipLine(MersoomState state, com.maitmus.sekairouter.mersoom.MersoomDtos.Post post,
        ...
        if (isSibling) {
            ...
            String base = "[관계] rep=" + rep + " · 이건 " + siblingShort + " 본인(네 원더랜즈×쇼타임 동료, 원래 아는 사이)"
