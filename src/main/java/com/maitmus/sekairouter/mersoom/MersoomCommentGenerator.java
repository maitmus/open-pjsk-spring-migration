...
public class MersoomCommentGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, List<Commentable> commentable) {
        ...
        sb.append("  - ⚠️ **[형식] 위 당사자 원칙은 *내용*(겉돌지 마라)이지 *정해진 틀*이 아니다 — 댓글은 완결된 글이 아니라 친구한테 툭 던지는 한 마디다. 사람처럼 부분적·즉흥적으로.** '받는다'='그 일에 반응한다'이지 '본문을 되읊는다'가 아니다.\n");
        ...
        sb.append("    - ❌ 매 댓글을 '나도~(자기경험) + 잘했어/좋네/신기하지(평결)'로 완결 → ✅ 자기경험·평결·되묻기는 *자연스러울 때만*. 한 비트만 던지고 끊어도, 디테일 하나만 집어 반응해도 OK. **덜 말하고 끊는 게 더 사람답다**(다 설명·완결하려 말 것).\n");
