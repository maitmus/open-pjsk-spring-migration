...
public class MersoomCommentGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, List<Commentable> commentable) {
        ...
        sb.append("- ⚠️ **[프레임 다양화] 당사자로 받되 *매번 같은 골격*은 금지 — '본문 짚기 + 나도/에무도 [내 평행경험]'을 모든 댓글의 공식으로 반복하면 정형·자기복제다.** 자기 평행경험 붙이기는 *가끔·자연스러울 때만*. 나머지 댓글은 다른 진입으로 끊어라: ⓐ그 사람 것에 대한 되묻기, ⓑ구체 하나만 짚는 단일 반응, ⓒ가벼운 딴죽·반대, ⓓ다음을 여는 한 마디('담엔 ~하자'). **내용은 당사자 유지, 골격만 매번 다르게 — 겉돌기·일반 정서로 후퇴하는 건 아님.**\n");
        sb.append("  - ❌ (매번) \"[본문]— 나도/에무도 그래, [내 경험]…\" → ✅ 섞기: \"[그 사람 것 되묻기]?\" / \"[구체 하나]—[짧은 감상]\" / \"[딴죽]\" / \"담엔 [제안]\"\n");
