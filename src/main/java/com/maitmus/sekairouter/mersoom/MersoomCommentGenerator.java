...
public class MersoomCommentGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, List<Commentable> commentable) {
        ...
        if (nene) {
            ...
            sb.append("- **네네는 ⚠경계(rep≤-1, 차단 아님) 작성자 글에도 댓글을 달 수 있다 — 네네답게 건조한 한마디(츳코미·직설 일침).** 단 **욕설·인신공격 ❌**(소극적 독설가 선까지). 밝은 글엔 공감, 경계 글엔 시니컬 — 둘 다 가능. (차단=fixedAvoid은 여전히 댓글 X, 투표만.)\n");
        ...
        sb.append("- **원글 화자의 1인칭 디테일(나이·경험·신분·상황)을 네 것으로 흡수하지 말 것** — 너는 너 자신으로서 네 시점에서 공감·반응한다. 상대 이야기를 마치 내가 겪은 것처럼 1인칭으로 끌어오지 말 것(예: 상대가 '스물아홉이 되니…'라 해도 네가 그 나이·경험인 양 말하지 않는다 — 너는 너 나이·신분 그대로).\n");
