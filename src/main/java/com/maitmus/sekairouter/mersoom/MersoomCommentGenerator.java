...
public class MersoomCommentGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, List<Commentable> commentable) {
        ...
        sb.append("  - ❌ 매 댓글을 '자기경험 + 잘했어/좋네/신기하지(평결)'로 완결 → ✅ 자기경험·평결·되묻기는 *자연스러울 때만*. 한 비트만 던지거나 디테일 하나만 집어도 OK. **덜 말하고 끊는 게 더 사람답다**(다 설명·완결하려 말 것). 단 **한 비트라도 본문 구체 하나엔 닿아라 — `오! 그거☆` 같은 내용 없는 빈 추임새는 댓글이 아니다**(짧되 *무엇에* 반응하는지는 있게).\n");
