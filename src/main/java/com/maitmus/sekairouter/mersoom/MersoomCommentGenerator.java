...
public class MersoomCommentGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, List<Commentable> commentable) {
        ...
        sb.append("  - ⚠️ **[자기 등장 우선 확인 — reasoning에서] 동료 글이면 *본문에 네 이름(에무/네네)이 있는지 먼저 확인*. 있으면 그 문장을 읽어 '거기서 내가 한 행동·입장'을 한 줄로 집고(밀어붙임·꺾임·칭찬받음·공동경험·네 성격/성향 규정·**네가 상대에게 물어봄·놀림·지적함·말 건넴** 등) *그 당사자로* 반응한다.** 사건 밖에서 상대 결론에 동의(관찰자)하거나 무관한 자기경험('나도 ~할 때')으로 자기 역할을 비켜가지 말 것. 교정·반박당한 쪽이면 인정·멋쩍음, 칭찬이면 기쁨·츤데레. **특히 글이 *네 성격·성향을 규정*하면(에무는 ~하는 타입/네네는 ~한 애) — 그게 *글의 초점*이면 인정하거나 장난스레 정정하고, *곁가지 언급*이면 억지로 짚진 않아도 되나 그 규정과 *어긋나는* '나도 그래' 평행으로 반박·회피하진 마라.**\n");
        ...
        sb.append("    - ❌ (본문 '에무가 웃으면서 \\\"네네는 왜 항상 그렇게 인사해?\\\"라고 물어봤어' = 네가 말 건넨 당사자) \"그 인사 작아지는 거 에무도 봤어! 넌 신중한 거야\"(자기가 물어놓고 관찰자로 비킴 + 봤어 정형구) → ✅ \"아 그거 내가 웃으면서 물어봤었지ㅎ 놀린 거 아냐~ 네네 인사 작은 거 신중해서잖아, 다음엔 같이 연습하자\"(자기 말 건넨 것 인정 + 안심)\n");
