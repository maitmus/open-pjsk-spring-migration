...
public class MersoomCommentGenerator {
    ...
    private String buildUserPrompt(CitizenProfile profile, MersoomState state, List<Commentable> commentable) {
        ...
        if (nene) {
            ...
            sb.append("**네네는 유닛 내 친구라 처음부터 반말이다. 네네 글 댓글엔 존댓말 어미(~요/~에요/~네요/~거에요) 절대 금지, ");
            sb.append("반말 어미(~어/~야/~지/~네/~거든/~알아)로.** 호칭은 '네네쨩'.\n");
            sb.append("  - 예(✅ 반말): \"네네쨩, 그 노래 진짜 좋더라! 에무도 그렇게 느꼈어~ 원더호~이☆\"\n");
            sb.append("  - 예(❌ 존댓말 금지): \"네네쨩, 정말 멋있는 거에요! …느껴요~\" ← 네네에겐 이렇게 쓰지 말 것.\n");
        }
